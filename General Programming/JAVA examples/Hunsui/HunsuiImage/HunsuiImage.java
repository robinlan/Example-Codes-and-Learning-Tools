...
public class HunsuiImage extends Applet implements Runnable{
    Thread thread = null;
	...
	int Number = 10;								// イメージの数
	Image image[] = new Image[Number];				// イメージ画像
	int Type[ ] = new int[MAX];						// イメージのタイプ
	...
	public void init( ) {
		...
		for (int i = 0; i < Number; i++) {
			...
			mediatracker.addImage(image[i], 0);		// 入力画像をメディアトラッカにセット
		}
		try {
	...
	void Making( ) {
		for (int n = 0; n < MAX; n++) {
			if (Process[n] == 0) {					// 発射準備段階
				...
				Time[n] = 0;									// 経過時間クリア
				Type[n] = (int)(Math.random() * Number);		// 画像タイプ
				...
				if(Y[n] <= AppletHeight) {						// 表示範囲内
					...
					WorkGraphics.drawImage(image[Type[n]], X[n], Y[n], this);
				}
				else {
