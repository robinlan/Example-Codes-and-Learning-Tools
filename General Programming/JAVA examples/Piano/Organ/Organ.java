...
public class Organ extends Applet implements MouseListener {
    ...
    public void init( ) {
		...
		WorkImage = createImage(AppletWidth, AppletHeight);	// 作業イメージ領域作成
		...
		for (int i = 12; i >= 0; i--) {					// 黒い鍵盤からチェック
			if (mouseX >= Point[i][0] && mouseX <= Point[i][0] + Point[i][2]
			 && mouseY >= Point[i][1] && mouseY <= Point[i][1] + Point[i][3]) {
				Keyboard[i] = 1;						// 今回の位置をセット
				Sound[i].loop( );						// 連続的に鳴らす
				PreHitKey = i;							// 今回の位置を記憶
				break;
			}
		}
        repaint( );
	...
	public void mouseReleased(MouseEvent evt) {
		Keyboard[PreHitKey] = 0;						// 前回位置をクリア
		Sound[PreHitKey].stop( );						// 演奏停止
        repaint( );										// 再描画
	}
