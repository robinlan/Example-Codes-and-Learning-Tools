function [trainRR, testRR]=htkTrainTest(htkPrm, printOpt)
% htkTrainTest: Use HTK for training and test
%	Usage: [trainRR, testRR]=htkTrainTest(htkPrm)

%	Roger Jang, 20060523

if nargin<1, htkPrm=htkParamSet; end
...
if printOpt, fprintf('I.2: Generate %s and %s\n', htkPrm.mnlFile, htkPrm.phoneMlfFile); end
fid=fopen('output\syl2phone.scp', 'w'); fprintf(fid, 'EX'); fclose(fid);
cmd=sprintf('HLEd -n output\\%s -d %s -l * -i output\\%s output\\syl2phone.scp %s', htkPrm.mnlFile, htkPrm.pamFile, htkPrm.phoneMlfFile, htkPrm.sylMlfFile);
dos(cmd);

if printOpt, fprintf('I.3: Generate wav2fea.scp¡G'); end
tic;
waveFiles=recursiveFileList(htkPrm.waveDir, 'wav');
...
tic;
cmd=sprintf('HCopy -C %s -S output\\wav2fea.scp', htkPrm.feaCfgFile);
dos(cmd);
%fprintf('%f sec\n', toc);
...
genTemplateHmmFile(htkPrm.feaType, htkPrm.feaDim, htkPrm.stateNum, outFile, mixtureNum, htkPrm.streamWidth);

if printOpt, fprintf('II.3: Populate template.hmm to generate hcompv.hmm\n'); end
cmd=sprintf('HCompV -m -o hcompv.hmm -M output -I output\\%s -S output\\trainFea.scp output\\template.hmm', htkPrm.phoneMlfFile);
dos(cmd);

if printOpt, fprintf('II.4: Duplidate hcompv.hmm to generate macro.init'); end
tic;
% Read modelName.txt
modelListFile=sprintf('output\\%s', htkPrm.mnlFile);;
...
fid=fopen('output\mxup.scp', 'w'); fprintf(fid, 'MU %d {*.state[2-4].mix}', htkPrm.mixtureNum); fclose(fid);
copyfile('output/macro.init', 'output/hmm/macro.0');
cmd=sprintf('HHEd -H output\\hmm\\macro.0 output\\mxup.scp output\\%s', htkPrm.mnlFile);
dos(cmd);
...
%	fprintf('%d/%d: Generate %s...\n', i, emCount, targetMacro);
	...
	cmd=sprintf('HERest -H %s -I output\\%s -S output\\trainFea.scp output\\%s', targetMacro, htkPrm.phoneMlfFile, htkPrm.mnlFile);
	dos(cmd);
end

if printOpt, fprintf('III.1: Use grammar file to generate net file\n'); end
cmd=sprintf('Hparse %s output\\digit.net', htkPrm.grammarFile);
dos(cmd);
...
cmd=sprintf('HVite -H %s -l * -i output\\result_test.mlf -w output\\digit.net -S output\\testFea.scp %s output\\%s', targetMacro, htkPrm.pamFile, htkPrm.mnlFile);
dos(cmd);
if printOpt, fprintf('\tInside test: Generating result_train.mlf\n'); end
cmd=sprintf('HVite -H %s -l * -i output\\result_train.mlf -w output\\digit.net -S output\\trainFea.scp %s output\\%s', targetMacro, htkPrm.pamFile, htkPrm.mnlFile);
dos(cmd);
...
dos(sprintf('findstr /v "sil" %s > output\\answer.mlf', htkPrm.sylMlfFile));
dos('findstr /v "sil" output\\result_test.mlf > output\\result_test_no_sil.mlf');
[status, result]=dos(sprintf('HResults -p -I output\\answer.mlf %s output\\result_test_no_sil.mlf > output\\outsideTest.txt', htkPrm.pamFile));
contents=fileread('output\outsideTest.txt');
keyStr='WORD: %Corr=';
startIndex=strfind(contents, keyStr)+length(keyStr);
testRR=eval(contents(startIndex:startIndex+4));
...
[status, result]=dos(sprintf('HResults -p -I output\\answer.mlf %s output\\result_train_no_sil.mlf > output\\insideTest.txt', htkPrm.pamFile));
contents=fileread('output\insideTest.txt');
keyStr='WORD: %Corr=';
startIndex=strfind(contents, keyStr)+length(keyStr);
trainRR=eval(contents(startIndex:startIndex+4));