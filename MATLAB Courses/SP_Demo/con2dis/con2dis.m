function con2dis(action)

% Last Update: 20-Oct-2001
...
if nargin == 0
   ...
   h =getuprop(gcbf,'Handles'); 	%getappdata(gcbf, 'DataStructure');
end


%globals
Ver = 'CON2DIS v1.02';  % Version string for figure title
...
switch action
 ...
 case 'Initialize'
  ...
  h = defaultplots(h,Ver);       % Create default plots
  ...
  %setappdata(gcf, 'DataStructure', h);
  setuprop(gcf, 'Handles', h);   % Store handles as new userproperty
...
case 'WindowButtonMotionFcn'
 ...
 case 'Editbox2'
  ...
  if h.wflag == 1
    ...
    set(h.Slider1,'Value',pi2*FoN);
    set(h.Slider1,'Max',1.49*pi2*FsN);
 ...
 case 'SetLineWidth'
  ...
  %setappdata(gcbf, 'DataStructure', h);
 ...
 case 'LineDragStart'
  ...
  setappdata(gcbf, 'CurrentX', CurrentPoint(1,1) );
 ...
 case 'MoveLine'    
  ...
  FoN = round(10*FoN)/10;   %- Round to nearest tenth
			    ...
			    setappdata(gcbf, 'CurrentX', CurrentX);
			    % PROBLEM setuprop(gcf, 'Handles', h);
  %====================================================================
 case 'LineDragStop'
  %====================================================================
  
  rmappdata(gcbf,'CurrentX');
  set(gcbf, 'WindowButtonMotionFcn', '');
  set(gcbf, 'WindowButtonUpFcn', '');
  setptr(gcbf,'arrow');
  set(gcbf, 'WindowButtonMotionFcn', 'con2dis WindowButtonMotionFcn');
  %====================================================================
 case 'PlotsMenu'   
  %====================================================================   
 ...
 case 'ShowW' 
  ...
  if strcmp(show,'off')
    ...
    set([h.Axis4,h.Axis6],...
	'XTickLabel',{sprintf('%.2f',-pi2*FsN),'','0','',sprintf('%.2f',pi2*FsN)} );        
    ...
    set([h.Axis4,h.Axis6],'XTickLabel',FsN*(-1:0.5:1) );         
 ...
 case 'ShowW_hat' 
  ...
  if strcmp(show_hat,'off')
    ...
    % change text labels
    h.FontSizeSym = get(h.Axis3,'FontSize');
    ...
    h.FontSize = get(h.Axis3,'FontSize');
    % change text labels
    set(gcf,'CurrentAxes',h.Axis3);
 ...
 case 'showLF'
  ...
  if strcmp(check,'off')
    ...
    set(h.RButton1,'Value',1);
  else
    set(h.ShowLF,'Checked','off');
    set(h.Line11,'Visible','on');
    set(h.Line52,'Visible','off');
    set(h.RButton1,'Value',0);
  end
  set(h.TextClick,'Visible','off','color','k');  %--changing the color is a flag to turn this off.

  %====================================================================
 case 'FontResize' % This is not finished yet
  %====================================================================
		   CurrentFontSizeT = get(h.TextFs,'FontSize');
		   CurrentFontSizeA = get(h.Axis1,'FontSize'); %font units get here 'normalized'
		   CurrentFontWtA = get(h.Axis1,'FontWeight');
		   CurrentFontWtT = get(h.TextFs,'FontWeight');
		   
		   if strcmp(CurrentFontWtA,'bold')
		     CurrentFontWtA = 1;
		   else
		     CurrentFontWtA = 0;
		   end
		   
		   if strcmp(CurrentFontWtT,'bold')
		     CurrentFontWtT = 1;
		   else
		     CurrentFontWtT = 0;
		   end
		   Vec = [CurrentFontSizeT CurrentFontWtT CurrentFontSizeA CurrentFontWtA];
		   set(0,'UserData',Vec);
		   [FontTnew,BoldTnew,FontAnew,BoldAnew] = axesfontdlg
		   
		   %set(0,'UserData',[AxesFontSize Boldit]);
		   ...
		   set(findobj(gcbf,'type','axes'),'FontSize',FontAnew, ...
				     'FontWeight',FontWeightA);
		   set(findobj(gcbf,'type','text'),'FontSize',FontTnew, ...
