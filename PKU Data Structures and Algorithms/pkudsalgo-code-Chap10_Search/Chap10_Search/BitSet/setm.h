...
public:	
	...
	friend ostream& operator<<(ostream& O, const mySet<N>& R);			//<<
...
template<size_t N> string mySet<N>::to_string() const{   // 用0，1形式把集合内容输出到字符串
	string S;
	S.reserve(N);
	for (size_t P = N; 0 < P; )
		S += test(--P) ? '1' : '0';
...
template<size_t N> mySet<N> operator|(const mySet<N>& L, const mySet<N>& R){  //并运算
	return (mySet<N>(L) |= R); 
}

template<size_t N> mySet<N> operator-(const mySet<N>& L, const mySet<N>& R){  //差运算
	return (mySet<N>(L) & ~R); 
}

template<size_t N> mySet<N> operator^(const mySet<N>& L, const mySet<N>& R){  //异或运算
	return (mySet<N>(L) ^= R); 
}

template<size_t N> ostream& operator<<(ostream& O, const mySet<N>& R){        //重载<<运算符
